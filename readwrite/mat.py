...
cols = df2.columns.drop(
    "date")  # here we are storing all the columns of df2 in cols variable except a column date which must be in the datetime format
df2[cols] = df2[cols].apply(pd.to_numeric,
                            errors='coerce')  # here we are converting all the columns data into numeric values except for the datetime
df2[cols] = df2[cols].fillna(0)  # here we filling the null values with 0 in all the columns except for the date one
df2.plot(kind="bar", x="date", figsize=(12,
                                        6))  # here we are using the pandas dataframe directly inorder to build a barchart where the x-axis value will be date
...
print(df2.head(3))
df3 = df2.set_index('date')  # here we are setting the date column as the index
print(df3.loc['01-01-2022'])  # here we are extracting the data using the index value which is from the date column
